...
# -*- coding: utf-8 -*-
"""
Optimized causal MV + camera -> TIP-style lazy depth reconstruction.

Input MV CSV columns:
  poc,x,y,w,h,list,ref_poc,mv_x,mv_y

Main temporal path:
  1) Compact each decoded picture's 4x4/subblock MVs once into a TIP-like
     processing-grid trajectory cache.
  2) For the current POC, project all cached decoded-source trajectories with
     vectorized temporal scaling and forward-splat them into current cells.
  3) Keep only a small top-K set per current cell, convert each correspondence
     directly to an inverse-depth observation with vectorized camera geometry,
     and fit only reachable target blocks.
  4) Do not reconstruct or warp a reference depth map in ``tip_mv`` mode.

The original decoded-depth propagation path remains available through
``--temporal-mode depth_warp`` and both paths can be compared with
``--temporal-mode both``.  GT depth is used only for encoder-side oracle
candidate selection in this simulation.

Key optimizations over v6:
  - fixed farther/nearer endpoint generation;
  - one-time source TMVP trajectory compaction;
  - NumPy-vectorized temporal projection and MV-to-depth solving;
  - vectorized cell top-K selection;
  - sparse reachable-block job construction;
  - bounded, reference-diverse fit observations;
  - vectorized plane rendering.
"""
...
# ============================================================
# TIP-style temporal MV projection field
# ============================================================

@dataclass
class TIPTrajectoryBatch:
    """Compact, prefiltered motion trajectories from one decoded picture.

    All coordinates and MVs are already expressed at processing resolution.
    Keeping the fields as contiguous NumPy arrays avoids rebuilding Python
    objects and rescanning the original CSV rows for every target picture.
    """
...
@dataclass
class TIPProjectedStats:
    cached_source_trajectories: int = 0
    source_trajectories_considered: int = 0
    bracketed_trajectories: int = 0
    extrapolated_trajectories: int = 0
    projected_endpoint_observations: int = 0
    rejected_outside: int = 0
    rejected_temporal: int = 0
    duplicate_candidates_removed: int = 0
    discarded_by_topk: int = 0
    kept_observations: int = 0
    valid_depth_observations: int = 0
    nonempty_cells: int = 0


def _list_code(value: str) -> int:
    text = str(value).strip().upper()
    if text in ("L0", "0", "LIST_0", "REF_PIC_LIST_0"):
        return 0
    if text in ("L1", "1", "LIST_1", "REF_PIC_LIST_1"):
        return 1
    # Stable fallback without Python's randomized hash().
    return 2 + sum((i + 1) * ord(ch) for i, ch in enumerate(text)) % 253


def _topk_indices_per_cell(
    cell_ids: np.ndarray,
    scores: np.ndarray,
    max_per_cell: int,
) -> np.ndarray:
    """Return score-descending top-K indices for every integer cell id."""
    if cell_ids.size == 0 or max_per_cell <= 0:
        return np.empty(0, dtype=np.int64)

    order = np.lexsort((-scores, cell_ids))
    sorted_cells = cell_ids[order]
    group_start = np.empty(sorted_cells.size, dtype=bool)
    group_start[0] = True
    group_start[1:] = sorted_cells[1:] != sorted_cells[:-1]
    starts = np.maximum.accumulate(
        np.where(group_start, np.arange(sorted_cells.size), 0)
    )
    rank = np.arange(sorted_cells.size) - starts
    return order[rank < max_per_cell]


def build_tip_trajectory_cache(
    mv_by_frame: Sequence[Sequence[MVObservation]],
    full_width: int,
    full_height: int,
    processing_width: int,
    processing_height: int,
    ...
    max_trajectories_per_source_cell: int,
    baseline_cap: float,
) -> Tuple[Dict[int, TIPTrajectoryBatch], Dict[str, Any]]:
    """Build a compact TIP-like source motion field once for the sequence.

    AV2 TIP does not repeatedly scan every subblock MV for every prediction.
    It first works on a compact TMVP field.  This function applies the same
    principle: each source-picture processing cell keeps only a few diverse,
    high-value trajectories, and all later target-picture projections operate
    on these contiguous arrays.
    """
    if source_cell_size <= 0:
        raise ValueError("TIP source_cell_size must be positive")
    if max_trajectories_per_source_cell < 0:
        raise ValueError("TIP max trajectories per source cell must be >= 0")

    scale = float(downsample_scale)
    grid_w = (processing_width + source_cell_size - 1) // source_cell_size
    cache: Dict[int, TIPTrajectoryBatch] = {}
    total_input = 0
    total_eligible = 0
    total_group_dedup = 0
    total_topk_discard = 0

    for source_poc, rows in enumerate(mv_by_frame):
        total_input += len(rows)
        if not rows:
            continue

        x_full = np.asarray(
            [r.x + (r.w - 1) * 0.5 for r in rows], dtype=np.float64
        )
        y_full = np.asarray(
            [r.y + (r.h - 1) * 0.5 for r in rows], dtype=np.float64
        )
        mv_x = np.asarray([r.mv_x for r in rows], dtype=np.float64) / scale
        mv_y = np.asarray([r.mv_y for r in rows], dtype=np.float64) / scale
        ref_poc = np.asarray([r.ref_poc for r in rows], dtype=np.int32)
        list_code = np.asarray([_list_code(r.list_id) for r in rows], dtype=np.uint8)

        x = (x_full + 0.5) / scale - 0.5
        y = (y_full + 0.5) / scale - 0.5
        dt = ref_poc.astype(np.int64) - int(source_poc)
        finite = (
            np.isfinite(x) & np.isfinite(y)
            & np.isfinite(mv_x) & np.isfinite(mv_y)
        )
        valid = (
            finite
            & (dt != 0)
            & (x_full >= 0.0) & (x_full < full_width)
            & (y_full >= 0.0) & (y_full < full_height)
            & (x >= -0.5) & (x < processing_width - 0.5)
            & (y >= -0.5) & (y < processing_height - 0.5)
        )
        if not np.any(valid):
            continue

        x = np.ascontiguousarray(x[valid])
        y = np.ascontiguousarray(y[valid])
        mv_x = np.ascontiguousarray(mv_x[valid])
        mv_y = np.ascontiguousarray(mv_y[valid])
        ref_poc = np.ascontiguousarray(ref_poc[valid])
        list_code = np.ascontiguousarray(list_code[valid])
        total_eligible += int(x.size)

        gx = np.floor(np.maximum(x, 0.0) / source_cell_size).astype(np.int32)
        gy = np.floor(np.maximum(y, 0.0) / source_cell_size).astype(np.int32)
        cell_id = gy.astype(np.int64) * grid_w + gx.astype(np.int64)
        baseline = np.minimum(
            np.abs(ref_poc.astype(np.float64) - float(source_poc)),
            max(float(baseline_cap), 1.0),
        )
        motion = np.minimum(np.hypot(mv_x, mv_y), 64.0) / 64.0
        score = baseline * (0.95 + 0.05 * motion)

        # Remove only exact duplicate span records. Keep additional samples
        # from the same ref/list group so a source cell may still retain
        # spatially different trajectories when the top-K budget permits it.
        qx = np.rint(x * 64.0).astype(np.int64)
        qy = np.rint(y * 64.0).astype(np.int64)
        qmx = np.rint(mv_x * 64.0).astype(np.int64)
        qmy = np.rint(mv_y * 64.0).astype(np.int64)
        exact_order = np.lexsort(
            (-score, qmy, qmx, qy, qx, list_code, ref_poc, cell_id)
        )
        exact_keys = (
            cell_id[exact_order], ref_poc[exact_order], list_code[exact_order],
            qx[exact_order], qy[exact_order], qmx[exact_order], qmy[exact_order],
        )
        exact_first = np.empty(exact_order.size, dtype=bool)
        exact_first[0] = True
        exact_first[1:] = False
        for key in exact_keys:
            exact_first[1:] |= key[1:] != key[:-1]
        unique = exact_order[exact_first]
        total_group_dedup += int(x.size - unique.size)

        x = x[unique]
        y = y[unique]
        mv_x = mv_x[unique]
        mv_y = mv_y[unique]
        ref_poc = ref_poc[unique]
        list_code = list_code[unique]
        cell_id = cell_id[unique]
        score = score[unique]

        # Give the best sample of every (cell, ref, list) group a large but
        # finite bonus. Top-K therefore preserves temporal/list diversity first
        # and fills remaining slots with the next strongest spatial samples.
        group_order = np.lexsort((-score, list_code, ref_poc, cell_id))
        sorted_cell = cell_id[group_order]
        sorted_ref = ref_poc[group_order]
        sorted_list = list_code[group_order]
        group_first = np.empty(group_order.size, dtype=bool)
        group_first[0] = True
        group_first[1:] = (
            (sorted_cell[1:] != sorted_cell[:-1])
            | (sorted_ref[1:] != sorted_ref[:-1])
            | (sorted_list[1:] != sorted_list[:-1])
        )
        representative_mask = np.zeros(score.size, dtype=bool)
        representative_mask[group_order[group_first]] = True
        priority = score + representative_mask.astype(np.float64) * (
            float(np.max(score)) + 1.0
        )

        if max_trajectories_per_source_cell > 0:
            keep = _topk_indices_per_cell(
                cell_id, priority, max_trajectories_per_source_cell
            )
            total_topk_discard += int(x.size - keep.size)
            x = x[keep]
            y = y[keep]
            mv_x = mv_x[keep]
            mv_y = mv_y[keep]
            ref_poc = ref_poc[keep]
            list_code = list_code[keep]

        cache[source_poc] = TIPTrajectoryBatch(
            source_poc=source_poc,
            x=np.ascontiguousarray(x, dtype=np.float64),
            y=np.ascontiguousarray(y, dtype=np.float64),
            mv_x=np.ascontiguousarray(mv_x, dtype=np.float64),
            mv_y=np.ascontiguousarray(mv_y, dtype=np.float64),
            ref_poc=np.ascontiguousarray(ref_poc, dtype=np.int32),
            list_code=np.ascontiguousarray(list_code, dtype=np.uint8),
        )

    kept = int(sum(batch.size for batch in cache.values()))
    stats = {
        "input_mv_rows": int(total_input),
        "eligible_mv_rows": int(total_eligible),
        "source_exact_duplicates_removed": int(total_group_dedup),
        "discarded_by_source_topk": int(total_topk_discard),
        "cached_trajectories": kept,
        "cached_source_pictures": int(len(cache)),
        "source_cell_size": int(source_cell_size),
        "max_trajectories_per_source_cell": int(max_trajectories_per_source_cell),
        "grid_width": int(grid_w),
    }
    return cache, stats


def _solve_depth_closed_form_vectorized(
    u: np.ndarray,
    v: np.ndarray,
    mv_x: np.ndarray,
    mv_y: np.ndarray,
    ...
    """Vectorized equivalent of ``solve_depth_closed_form_scalar``."""
    ur = u + mv_x
    vr = v + mv_y
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        ...
        pred_u = rel.dst_fx * x_ref / ref_depth + rel.dst_cx
        pred_v = rel.dst_fy * y_ref / ref_depth + rel.dst_cy
        error = np.hypot(pred_u - ur, pred_v - vr)

    valid = (
        np.isfinite(depth) & np.isfinite(error) & np.isfinite(ref_depth)
        & (denom >= min_parallax * min_parallax)
        & (depth >= min_depth) & (depth <= max_depth)
        & (ref_depth > 1e-10)
        & (error <= max_reproj_error)
    )
    return depth, error, valid


def _deduplicate_projected_candidates(
    cell_id: np.ndarray,
    ref_poc: np.ndarray,
    x: np.ndarray,
    y: np.ndarray,
    ...
    """Keep the highest-score candidate for each quantized correspondence."""
    if x.size == 0:
        return np.empty(0, dtype=np.int64)
    qx = np.rint(x * 16.0).astype(np.int64)
    qy = np.rint(y * 16.0).astype(np.int64)
    qmx = np.rint(mv_x * 16.0).astype(np.int64)
    qmy = np.rint(mv_y * 16.0).astype(np.int64)
    order = np.lexsort((-scores, qmy, qmx, qy, qx, ref_poc, cell_id))
    keys = (cell_id[order], ref_poc[order], qx[order], qy[order], qmx[order], qmy[order])
...
def build_tip_projected_depth_observations(
    trajectory_cache: Dict[int, TIPTrajectoryBatch],
    decoded_pocs: Sequence[int],
    target_poc: int,
    processing_width: int,
    processing_height: int,
    cell_size: int,
    max_observations_per_cell: int,
    endpoint_mode: str,
    ...
    min_parallax: float,
    max_reproj_error: float,
) -> Tuple[List[DepthObservation], Dict[str, Any]]:
    """Project compact source trajectories and solve current depth in batches.

    The function performs no reference-depth reconstruction.  It creates a
    reverse index by forward-splatting trajectory positions into target cells,
    selects cell-local top-K observations, and then solves depth vectorially for
    each current-to-endpoint camera pair.
    """
    if cell_size <= 0:
        raise ValueError("TIP cell_size must be positive")
    if max_observations_per_cell <= 0:
        return [], TIPProjectedStats().__dict__.copy()

    grid_w = (processing_width + cell_size - 1) // cell_size
    decoded_set = {int(p) for p in decoded_pocs}
    stats = TIPProjectedStats(
        cached_source_trajectories=int(
            sum(trajectory_cache[p].size for p in decoded_set if p in trajectory_cache)
        )
    )
    ...
    def append_candidates(
        ...
        mxs.append(mv_x)
        mys.append(mv_y)
        refs.append(ref_poc.astype(np.int32, copy=False))
        endpoint_tags.append(np.full(x.size, tag, dtype=np.uint8))
        alphas.append(alpha)

    for source_poc in decoded_pocs:
        batch = trajectory_cache.get(int(source_poc))
        if batch is None or batch.size == 0:
            continue
        stats.source_trajectories_considered += batch.size

        dt = batch.ref_poc.astype(np.float64) - float(batch.source_poc)
        alpha = (float(target_poc) - float(batch.source_poc)) / dt
        bracketed = (alpha >= 0.0) & (alpha <= 1.0)
        temporal_valid = bracketed.copy()
        if allow_extrapolation:
            temporal_valid |= (
                (alpha >= -max_extrapolation_ratio)
                & (alpha <= 1.0 + max_extrapolation_ratio)
            )
        stats.bracketed_trajectories += int(np.count_nonzero(bracketed))
        stats.extrapolated_trajectories += int(
            np.count_nonzero(temporal_valid & ~bracketed)
        )
        stats.rejected_temporal += int(batch.size - np.count_nonzero(temporal_valid))
        if not np.any(temporal_valid):
            continue

        alpha_v = alpha[temporal_valid]
        x = batch.x[temporal_valid] + alpha_v * batch.mv_x[temporal_valid]
        y = batch.y[temporal_valid] + alpha_v * batch.mv_y[temporal_valid]
        base_mx = batch.mv_x[temporal_valid]
        base_my = batch.mv_y[temporal_valid]
        ref = batch.ref_poc[temporal_valid]
        inside = (
            np.isfinite(x) & np.isfinite(y)
            & (x >= -0.5) & (x < processing_width - 0.5)
            & (y >= -0.5) & (y < processing_height - 0.5)
        )
        stats.rejected_outside += int(x.size - np.count_nonzero(inside))
        if not np.any(inside):
            continue

        x = x[inside]
        y = y[inside]
        base_mx = base_mx[inside]
        base_my = base_my[inside]
        ref = ref[inside]
        alpha_v = alpha_v[inside]

        source_ref = np.full(x.size, batch.source_poc, dtype=np.int32)
        source_mx = -alpha_v * base_mx
        source_my = -alpha_v * base_my
        ref_mx = (1.0 - alpha_v) * base_mx
        ref_my = (1.0 - alpha_v) * base_my
        source_ok = source_ref != target_poc
        ref_ok = ref != target_poc

        if endpoint_mode == "source":
            append_candidates(
                x[source_ok], y[source_ok], source_mx[source_ok], source_my[source_ok],
                source_ref[source_ok], 0, alpha_v[source_ok],
            )
        elif endpoint_mode == "reference":
            append_candidates(
                x[ref_ok], y[ref_ok], ref_mx[ref_ok], ref_my[ref_ok],
                ref[ref_ok], 1, alpha_v[ref_ok],
            )
        ...
        elif endpoint_mode in ("farther", "nearer"):
            source_distance = np.full(x.size, abs(batch.source_poc - target_poc), dtype=np.int32)
            ref_distance = np.abs(ref.astype(np.int64) - int(target_poc))
            choose_ref = (
                ref_distance > source_distance
                if endpoint_mode == "farther"
                else ref_distance < source_distance
            )
            choose_ref &= ref_ok
            choose_ref |= ~source_ok
            valid_endpoint = source_ok | ref_ok
            chosen_ref = np.where(choose_ref, ref, source_ref)
            chosen_mx = np.where(choose_ref, ref_mx, source_mx)
            chosen_my = np.where(choose_ref, ref_my, source_my)
            chosen_tag = np.where(choose_ref, 1, 0).astype(np.uint8)
            if np.any(valid_endpoint):
                xs.append(x[valid_endpoint])
                ys.append(y[valid_endpoint])
                mxs.append(chosen_mx[valid_endpoint])
                mys.append(chosen_my[valid_endpoint])
                refs.append(chosen_ref[valid_endpoint].astype(np.int32, copy=False))
                endpoint_tags.append(chosen_tag[valid_endpoint])
                alphas.append(alpha_v[valid_endpoint])
        else:
            raise ValueError(f"Unsupported TIP endpoint mode: {endpoint_mode}")

    if not xs:
        result = stats.__dict__.copy()
        result.update({
            "grid_width": int(grid_w),
            "grid_height": int((processing_height + cell_size - 1) // cell_size),
            "cell_size": int(cell_size),
            "max_observations_per_cell": int(max_observations_per_cell),
    ...
    x = np.concatenate(xs)
    y = np.concatenate(ys)
    mv_x = np.concatenate(mxs)
    mv_y = np.concatenate(mys)
    ref_poc = np.concatenate(refs)
    tag = np.concatenate(endpoint_tags)
    alpha = np.concatenate(alphas)
    stats.projected_endpoint_observations = int(x.size)

    gx = np.floor(np.maximum(x, 0.0) / cell_size).astype(np.int32)
    gy = np.floor(np.maximum(y, 0.0) / cell_size).astype(np.int32)
    cell_id = gy.astype(np.int64) * grid_w + gx.astype(np.int64)
    baseline = np.minimum(
        np.abs(ref_poc.astype(np.float64) - float(target_poc)),
        max(float(baseline_cap), 1.0),
    )
    center_x = gx.astype(np.float64) * cell_size + (cell_size - 1) * 0.5
    center_y = gy.astype(np.float64) * cell_size + (cell_size - 1) * 0.5
    dist2 = (x - center_x) ** 2 + (y - center_y) ** 2
    center_score = 1.0 / (1.0 + dist2 / max(float(cell_size * cell_size), 1.0))
    motion_score = np.minimum(np.hypot(mv_x, mv_y), 64.0) / 64.0
    extrapolation = np.maximum.reduce((np.zeros_like(alpha), -alpha, alpha - 1.0))
    temporal_score = 1.0 / (1.0 + 4.0 * extrapolation)
    score = (
        baseline
        * (0.72 + 0.28 * center_score)
        * (0.90 + 0.10 * motion_score)
        * temporal_score
    )

    unique_keep = _deduplicate_projected_candidates(
        cell_id, ref_poc, x, y, mv_x, mv_y, score
    )
    stats.duplicate_candidates_removed = int(x.size - unique_keep.size)
    x = x[unique_keep]
    y = y[unique_keep]
    mv_x = mv_x[unique_keep]
    mv_y = mv_y[unique_keep]
    ref_poc = ref_poc[unique_keep]
    tag = tag[unique_keep]
    cell_id = cell_id[unique_keep]
    score = score[unique_keep]

    keep = _topk_indices_per_cell(cell_id, score, max_observations_per_cell)
    stats.discarded_by_topk = int(x.size - keep.size)
    x = x[keep]
    y = y[keep]
    mv_x = mv_x[keep]
    mv_y = mv_y[keep]
    ...
    score = score[keep]
    stats.kept_observations = int(x.size)
    stats.nonempty_cells = int(np.unique(cell_id).size)

    observations: List[DepthObservation] = []
    for endpoint_poc in np.unique(ref_poc):
        rel = relative_cache.get((int(target_poc), int(endpoint_poc)))
        if rel is None:
            continue
        idx = np.flatnonzero(ref_poc == endpoint_poc)
        depth, error, valid = _solve_depth_closed_form_vectorized(
            x[idx], y[idx], mv_x[idx], mv_y[idx], rel,
            min_parallax=min_parallax,
            max_reproj_error=max_reproj_error,
            min_depth=min_depth,
            max_depth=max_depth,
        ...
        good = idx[valid]
        for j, d, e in zip(good.tolist(), depth[valid].tolist(), error[valid].tolist()):
            observations.append(
                DepthObservation(
                    poc=int(target_poc),
                    x=float(x[j]),
                    y=float(y[j]),
                    depth=float(d),
                    reproj_error=float(e),
                    ref_poc=int(ref_poc[j]),
                    list_id="TIP_REF" if int(tag[j]) else "TIP_SRC",
                )
            )

    stats.valid_depth_observations = len(observations)
    result = stats.__dict__.copy()
    result.update({
        "grid_width": int(grid_w),
        "grid_height": int((processing_height + cell_size - 1) // cell_size),
        ...
        "vectorized_projection": True,
        "vectorized_depth_solve": True,
    })
...
def _limit_depth_observations_for_fit(
    observations: Sequence[DepthObservation],
    budget: int,
    cx: float,
    cy: float,
) -> List[DepthObservation]:
    """Keep a small, reference-diverse and spatially distributed fit set."""
    ...
    # First keep the most accurate observation from each temporal/list group.
    groups: Dict[Tuple[int, str], List[DepthObservation]] = {}
    for obs in remaining:
        groups.setdefault((obs.ref_poc, obs.list_id), []).append(obs)
    group_order = sorted(
        groups.items(),
        key=lambda kv: (
            min(o.reproj_error for o in kv[1]),
            -abs(kv[0][0] - kv[1][0].poc),
            str(kv[0][1]),
        ),
    )
    for _, group in group_order:
        if len(selected) >= budget:
            break
        chosen = min(
            group,
    ...
    # Fill remaining slots by spatial coverage with a mild reprojection penalty.
    while remaining and len(selected) < budget:
        if selected:
            def score(obs: DepthObservation) -> Tuple[float, float, float]:
                min_dist = min(
                    (obs.x - old.x) ** 2 + (obs.y - old.y) ** 2
                    for old in selected
                )
                return (
                    min_dist / (1.0 + obs.reproj_error * obs.reproj_error),
                    -obs.reproj_error,
                    -((obs.x - cx) ** 2 + (obs.y - cy) ** 2),
                )
        else:
            def score(obs: DepthObservation) -> Tuple[float, float, float]:
                return (
                    1.0 / (1.0 + obs.reproj_error * obs.reproj_error),
                    -obs.reproj_error,
                    -((obs.x - cx) ** 2 + (obs.y - cy) ** 2),
                )
...
def build_block_fit_jobs_from_offsets(
    ...
    max_points_per_job: int = 0,
    allowed_target_blocks: Optional[set[Tuple[int, int]]] = None,
) -> List[Dict[str, Any]]:
    """Build only reachable fit jobs instead of scanning the complete grid.

    A source observation cell can contribute only to target cells implied by
    ``source_offsets``.  Deriving those target keys makes the operation sparse
    and preserves the lazy character of the projected-MV path.
    """
    if not observations:
        return []
    ...
    for sx, sy in block_obs:
        for dx, dy in source_offsets:
            tx = sx - dx
            ty = sy - dy
            if 0 <= tx < grid_w and 0 <= ty < grid_h:
                key = (tx, ty)
                if allowed_target_blocks is None or key in allowed_target_blocks:
                    target_keys.add(key)

    jobs: List[Dict[str, Any]] = []
    for gx, gy in sorted(target_keys, key=lambda k: (k[1], k[0])):
        bx = gx * fit_block
        by = gy * fit_block
        bw = min(fit_block, width - bx)
        bh = min(fit_block, height - by)
        ...
        if len(selected) < min_points:
            continue
        selected = _limit_depth_observations_for_fit(
            selected, max_points_per_job, cx, cy
        )
        if len(selected) < min_points:
            continue
        ...
        jobs.append({
            ...
            "xs": np.fromiter((o.x for o in selected), dtype=np.float64),
            "ys": np.fromiter((o.y for o in selected), dtype=np.float64),
            "depths": np.fromiter((o.depth for o in selected), dtype=np.float64),
            "errors": np.fromiter((o.reproj_error for o in selected), dtype=np.float64),
        })
    return jobs
...
def fit_and_render_local_candidate(
    ...
    jobs = build_block_fit_jobs_from_offsets(
        ...
        max_points_per_job=max_points_per_job,
        allowed_target_blocks=allowed_target_blocks,
    )
    ...
    stats = {
        ...
        "max_points_per_job": int(max_points_per_job),
...
def render_jobs(
    ...
    """Render each fitted block with NumPy broadcasting, not Python pixels."""
    ...
    for job, coeff in zip(jobs, coeffs):
        ...
        x_offsets = np.arange(bw, dtype=np.float64) + (bx - cx)
        y_offsets = np.arange(bh, dtype=np.float64) + (by - cy)
        inv_block = c + a * x_offsets[None, :] + b * y_offsets[:, None]
        block_valid = np.isfinite(inv_block) & (inv_block > min_inv_depth)
        block_depth = np.zeros_like(inv_block)
        with np.errstate(divide="ignore", invalid="ignore"):
            block_depth[block_valid] = 1.0 / inv_block[block_valid]
        block_valid &= (block_depth >= min_depth) & (block_depth <= max_depth)
        block_depth[~block_valid] = 0.0

        dst_depth = depth[by:by + bh, bx:bx + bw]
        dst_valid = valid[by:by + bh, bx:bx + bw]
        dst_conf = confidence[by:by + bh, bx:bx + bw]
        dst_depth[block_valid] = block_depth[block_valid]
        dst_valid[block_valid] = True
        dst_conf[block_valid] = block_conf
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=(
            "Optimized TIP-style projected-MV lazy depth reconstruction with "
            "optional decoded-depth propagation and GT-oracle selection."
        )
    )
    ...
    ap.add_argument(
        "--temporal-mode", choices=("tip_mv", "depth_warp", "both", "none"),
        default="tip_mv",
        help="Temporal candidate path. tip_mv avoids reference-depth warping.",
    )
    ap.add_argument("--tip-cell-size", type=int, default=0,
                    help="TIP projected-MV cell size in processing pixels; 0 uses processing fit-block.")
    ap.add_argument("--tip-source-cell-size", type=int, default=0,
                    help="Source TMVP compaction cell size in processing pixels; 0 uses --tip-cell-size.")
    ap.add_argument("--tip-source-max-trajectories-per-cell", type=int, default=2,
                    help="Maximum cached source trajectories per source cell; 0 keeps all group representatives.")
    ap.add_argument("--tip-max-observations-per-cell", type=int, default=4)
    ap.add_argument("--max-depth-observations-per-fit", type=int, default=16,
                    help="Maximum depth observations used by one inverse-depth fit; 0 keeps all.")
    ap.add_argument("--tip-endpoint-mode",
                    choices=("both", "source", "reference", "farther", "nearer"),
                    default="farther",
                    help="Which endpoint(s) of each projected MV trajectory become current-to-reference observations.")
    ap.add_argument("--tip-neighbor-radius", type=int, default=1,
                    help="Use projected observations from current cell and this many neighboring cells.")
    ap.add_argument("--tip-allow-extrapolation", action="store_true", default=False)
    ap.add_argument("--tip-max-extrapolation-ratio", type=float, default=0.25)
    ap.add_argument("--tip-baseline-cap", type=float, default=32.0)
    ...
    if args.tip_source_cell_size < 0:
        raise ValueError("--tip-source-cell-size must be >= 0")
    if args.tip_source_max_trajectories_per_cell < 0:
        raise ValueError("--tip-source-max-trajectories-per-cell must be >= 0")
    if args.tip_max_observations_per_cell < 0:
        raise ValueError("--tip-max-observations-per-cell must be >= 0")
    if args.max_depth_observations_per_fit < 0:
        raise ValueError("--max-depth-observations-per-fit must be >= 0")
    if args.tip_neighbor_radius < 0:
        raise ValueError("--tip-neighbor-radius must be >= 0")
    if args.tip_max_extrapolation_ratio < 0.0:
        raise ValueError("--tip-max-extrapolation-ratio must be >= 0")
    ...
    tip_cell_size = processing_fit_block if args.tip_cell_size == 0 else args.tip_cell_size
    if tip_cell_size <= 0:
        raise ValueError("--tip-cell-size must be positive")
    tip_source_cell_size = (
        tip_cell_size if args.tip_source_cell_size == 0
        else args.tip_source_cell_size
    )
    if tip_source_cell_size <= 0:
        raise ValueError("--tip-source-cell-size must be positive")
    tip_source_offsets = tuple(
        (dx, dy)
        for dy in range(-args.tip_neighbor_radius, args.tip_neighbor_radius + 1)
        for dx in range(-args.tip_neighbor_radius, args.tip_neighbor_radius + 1)
    )
    geometry_block_size = (
    ...
    tip_cache_start = perf_counter()
    if args.temporal_mode in ("tip_mv", "both"):
        tip_trajectory_cache, tip_trajectory_cache_stats = build_tip_trajectory_cache(
            mv_by_frame=mv_by_frame,
            full_width=args.width,
            full_height=args.height,
            processing_width=processing_width,
            processing_height=processing_height,
            downsample_scale=args.downsample_scale,
            source_cell_size=tip_source_cell_size,
            max_trajectories_per_source_cell=args.tip_source_max_trajectories_per_cell,
            baseline_cap=args.tip_baseline_cap,
        )
    else:
        tip_trajectory_cache = {}
        tip_trajectory_cache_stats = {"enabled": False}
    tip_cache_seconds = perf_counter() - tip_cache_start
    ...
    print(f"TIP projected-MV grid     : cell {tip_cell_size}, top-{args.tip_max_observations_per_cell}, neighbor radius {args.tip_neighbor_radius}")
    print(f"TIP source TMVP cache     : cell {tip_source_cell_size}, top-{args.tip_source_max_trajectories_per_cell or 'all'}")
    print(f"TIP cached trajectories   : {tip_trajectory_cache_stats.get('cached_trajectories', 0)} ({tip_cache_seconds:.3f}s)")
    print(f"TIP trajectory endpoint   : {args.tip_endpoint_mode}")
    print(f"depth obs / plane fit     : {args.max_depth_observations_per_fit or 'all'}")
    print("propagation causality     : decoded source motion fields only")
    ...
    for decode_rank, poc in enumerate(decode_order):
        ...
        for local_index, local_name in enumerate(local_candidate_names):
            local_depth_i, local_valid_i, local_conf_i, local_stats_i = (
                fit_and_render_local_candidate(
                    ...
                    max_points_per_job=args.max_depth_observations_per_fit,
                )
            )
            candidate = DepthCandidate(
        ...
        tip_stats: Dict[str, Any] = {"enabled": False}
        t_tip = 0.0

        if args.temporal_mode in ("tip_mv", "both"):
            tip_start = perf_counter()
            tip_depth_obs, tip_projection_stats = build_tip_projected_depth_observations(
                trajectory_cache=tip_trajectory_cache,
                decoded_pocs=list(state_bank.keys()),
                target_poc=poc,
                processing_width=processing_width,
                processing_height=processing_height,
                cell_size=tip_cell_size,
                max_observations_per_cell=args.tip_max_observations_per_cell,
                endpoint_mode=args.tip_endpoint_mode,
                allow_extrapolation=args.tip_allow_extrapolation,
                max_extrapolation_ratio=args.tip_max_extrapolation_ratio,
                baseline_cap=args.tip_baseline_cap,
                relative_cache=relative_cache,
                min_depth=args.min_depth,
                max_depth=args.max_depth,
            ...
            tip_depth, tip_valid, tip_conf, tip_fit_stats = fit_and_render_local_candidate(
                observations=tip_depth_obs, source_offsets=tip_source_offsets,
                width=processing_width, height=processing_height,
                fit_block=tip_cell_size, min_points=args.min_points,
            ...
            raw_candidates.append(DepthCandidate(
                name="tip_projected_mv", label=len(local_candidate_names),
                source_poc=None, depth=tip_depth, confidence=tip_conf,
            ))
            t_tip = perf_counter() - tip_start
            tip_stats = {
                "enabled": True, "projection": tip_projection_stats,
                "valid_depth_observations": len(tip_depth_obs),
                "fit": tip_fit_stats,
                "valid_pixel_ratio": float(np.mean(tip_valid)),
            }

        if args.temporal_mode in ("depth_warp", "both"):
            warp_sources = selected_sources
        else:
            warp_sources = []
        ...
        t_warp = max(0.0, perf_counter() - stage_start - t_tip)
        ...
        print_progress(
            ...
            num_sources=len(warp_sources) + (1 if tip_stats.get("enabled") else 0),
        )
        if args.profile:
            print(
                f"\n  time poc={poc:3d}: sample={t_mv_sampling:.3f}s "
                f"mv={t_mv_depth:.3f}s local={t_local:.3f}s "
                f"tip={t_tip:.3f}s warp={t_warp:.3f}s geom={t_geometry:.3f}s "
        ...
        json.dump(
            {
                ...
                "tip_cell_size_processing": tip_cell_size,
                "tip_source_cell_size_processing": tip_source_cell_size,
                "tip_source_max_trajectories_per_cell": args.tip_source_max_trajectories_per_cell,
                "tip_trajectory_cache_build_seconds": tip_cache_seconds,
                "tip_trajectory_cache": tip_trajectory_cache_stats,
                "tip_max_observations_per_cell": args.tip_max_observations_per_cell,
                "max_depth_observations_per_fit": args.max_depth_observations_per_fit,
                "tip_endpoint_mode": args.tip_endpoint_mode,
                "tip_neighbor_radius": args.tip_neighbor_radius,
                "tip_allow_extrapolation": args.tip_allow_extrapolation,
                "tip_max_extrapolation_ratio": args.tip_max_extrapolation_ratio,
                "tip_baseline_cap": args.tip_baseline_cap,
                "propagation_causality": "decoded-source-motion-field projection; optional decoded-depth warp",
