        ...
        for line_no, line in enumerate(text.splitlines(), start=1):
            ...
            try:
                ...
                raise RuntimeError(
                    ...
                    f"JSON error={e}\n"
                    f"JSONL error at line {line_no}: {e2}"
                ) from e2

        if not entries:
            raise RuntimeError(f"Camera parameter file is empty: {path}")
...
def has_camera_matrices(obj):
    ...
    return (
        bool(keys & {"InvProjectionMatrix", "invProjectionMatrix"})
        and bool(keys & {"ProjectionMatrix", "projectionMatrix"})
        and bool(keys & {"WorldToCameraMatrix", "worldToCameraMatrix"})
        and bool(keys & {
            "CameraToWorldMatrix",
            "cameraToWorldMatrix",
            "CameraToWorldMarix",
...
def camera_frame_poc(entry, fallback_idx):
    for key in ["frames", "frame", "frameIdx", "frame_idx", "poc", "POC"]:
        if key in entry:
            try:
...
def extract_camera_entries(cams_json):
    if isinstance(cams_json, list):
        ...
        if "frames" in cams_json and isinstance(cams_json["frames"], list):
            entries = cams_json["frames"]
        elif has_camera_matrices(cams_json):
            entries = [cams_json]
        else:
            numeric_items = []
...
def get_near_clip(entry):
    return float(get_value_by_alias(entry, ["nearClipPlane", "NearClipPlane"]))


def get_matrix(frame_entry, logical_name):
    aliases = {
        "InvProjectionMatrix": [
            "InvProjectionMatrix",
            "invProjectionMatrix",
    ...
    obj = get_value_by_alias(frame_entry, aliases[logical_name], required=True)
    ...
    if isinstance(obj, dict):
        ...
        for r in range(4):
            for c in range(4):
                key = f"e{r}{c}"
                if key not in obj:
                    raise KeyError(f"Missing key {logical_name}.{key}")
    ...
    if mat.shape != (4, 4):
        raise ValueError(f"{logical_name} shape is {mat.shape}, expected 4x4")

    return mat


def make_ndc_grid(w, h):
    xs = np.arange(w, dtype=np.float32)
    ys = np.arange(h, dtype=np.float32)
    xx, yy = np.meshgrid(xs, ys)

    x_ndc = (xx + 0.5) / w * 2.0 - 1.0
    y_ndc = 1.0 - (yy + 0.5) / h * 2.0
...
def forward_project_depth_to_target_view(
    ...
    P_src_inv = get_matrix(cam_source, "InvProjectionMatrix")
    C2W_src = get_matrix(cam_source, "CameraToWorldMatrix")
    W2C_tgt = get_matrix(cam_target, "WorldToCameraMatrix")
    P_tgt = get_matrix(cam_target, "ProjectionMatrix")

    x_ndc, y_ndc = make_ndc_grid(w, h)
    z_src = source_depth_linear.astype(np.float32)
    ...
    p_view_h = p_ndc @ P_src_inv.T
    p_view = p_view_h[..., :3] / np.maximum(p_view_h[..., 3:4], 1e-8)

    denom = np.maximum(np.abs(p_view[..., 2:3]), 1e-8)
    p_view_scaled = p_view / denom * z_src[..., None]

    ones = np.ones((h, w, 1), dtype=np.float32)
    p_src_view4 = np.concatenate([p_view_scaled, ones], axis=-1)

    p_world = p_src_view4 @ C2W_src.T
    p_tgt_cam = p_world @ W2C_tgt.T

    p_clip_tgt = p_tgt_cam @ P_tgt.T
    w_clip = p_clip_tgt[..., 3]
    good_w = np.abs(w_clip) > 1e-8

    ndc_x_tgt = p_clip_tgt[..., 0] / np.where(good_w, w_clip, 1.0)
    ndc_y_tgt = p_clip_tgt[..., 1] / np.where(good_w, w_clip, 1.0)

    map_x = (ndc_x_tgt + 1.0) * 0.5 * w - 0.5
    map_y = (1.0 - ndc_y_tgt) * 0.5 * h - 0.5
    z_tgt = np.abs(p_tgt_cam[..., 2]).astype(np.float32)
    ...
    if splat_mode == "nearest":
        xi = np.rint(map_x[valid_src]).astype(np.int32)
        yi = np.rint(map_y[valid_src]).astype(np.int32)
        zi = z_tgt[valid_src].astype(np.float32)

        xi = np.clip(xi, 0, w - 1)
        yi = np.clip(yi, 0, h - 1)

        np.minimum.at(zbuf, yi * w + xi, zi)

    else:
        mx = map_x[valid_src].astype(np.float32)
        my = map_y[valid_src].astype(np.float32)
        zi = z_tgt[valid_src].astype(np.float32)
        ...
        for dy in [0, 1]:
            for dx in [0, 1]:
                ...
                np.minimum.at(zbuf, yi[ok] * w + xi[ok], zi[ok])

    target_depth = zbuf.reshape(h, w)
    valid = np.isfinite(target_depth)
    target_depth[~valid] = 0.0

    return target_depth, valid


def fill_depth_holes_nearest(depth, valid):
    if valid.all():
        return depth.copy()

    if not valid.any():
        raise RuntimeError("No valid pixels for hole filling")
    ...
    try:
        ...
        filled = depth.copy()
        filled[~valid] = depth[yy[~valid], xx[~valid]]
        return filled

    except Exception:
        return fill_depth_holes_iterative_mean(depth, valid)


def fill_depth_holes_iterative_mean(depth, valid, max_iter=2048):
    filled = depth.astype(np.float32).copy()
    cur = valid.astype(bool).copy()
    ...
    for _ in range(max_iter):
        ...
        for dy in [-1, 0, 1]:
            for dx in [-1, 0, 1]:
                ...
                shifted_v = np.roll(cur, shift=(dy, dx), axis=(0, 1))
                shifted_d = np.roll(filled, shift=(dy, dx), axis=(0, 1))
                ...
                acc += shifted_d * shifted_v
                cnt += shifted_v.astype(np.float32)
        ...
        filled[fillable] = acc[fillable] / cnt[fillable]
        cur[fillable] = True

    if not cur.all():
        filled[~cur] = float(np.median(filled[cur]))

    return filled


def make_warp_candidate_depth_y(prev_recon_y, cam_prev, cam_cur, w, h, max_value, splat_mode):
    near_prev = get_near_clip(cam_prev)
    near_cur = get_near_clip(cam_cur)

    src_linear = prev_recon_y.astype(np.float32) * near_prev

    tgt_linear_sparse, valid = forward_project_depth_to_target_view(
        source_depth_linear=src_linear,
        cam_source=cam_prev,
        cam_target=cam_cur,
        w=w,
        h=h,
    ...
    tgt_linear = fill_depth_holes_nearest(tgt_linear_sparse, valid)

    tgt_y = tgt_linear / max(near_cur, 1e-8)
    tgt_y = np.clip(np.rint(tgt_y), 0, max_value).astype(np.float64)
...
def make_candidates(
    ...
    if warp_padded is not None:
        warp_block = warp_padded[y : y + h, x : x + w]
        p = fit_plane(warp_block, pinv, cx, cy)
        conv["warp"] = p
        cand.append(("warp", p))
...
def encode_node(padded, x, y, w, h, depth, parent, args, grid, store, prev_store, warp_padded, adaptive):
    qt_ok = (
...
def simulate_one_frame(
    ...
    if warp_depth is not None:
        warp_padded, _, _ = pad_to_block_multiple(warp_depth, args.block_size)
    else:
        warp_padded = None
    ...
    summary = {
        ...
        "warp_candidate_available": 1 if warp_depth is not None else 0,
        "bits": total_bits,
        "bpp": total_bits / (h * w),
        "sse": total_sse,
        **m,
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Depth plane compression simulation with recursive split and camera-warp candidate."
    )
    ...
    p.add_argument("--camera-warp-candidate", action="store_true")
    p.add_argument("--camera-param", default="", help="Camera parameter JSON/JSONL/TXT path")
    p.add_argument("--splat-mode", choices=["nearest", "bilinear"], default="bilinear")
...
def main():
    ...
    if args.camera_warp_candidate and not args.camera_param:
        raise ValueError("--camera-warp-candidate requires --camera-param")
    ...
    if args.camera_warp_candidate:
        cams_json = load_camera_json(args.camera_param)
        camera_lookup, _ = build_camera_lookup(cams_json)
    ...
    try:
        with open(args.input, "rb") as fp:
            for fi in range(args.start_frame, end):
                ...
                if args.camera_warp_candidate and prev_recon_y is not None:
                    cam_prev = get_camera(camera_lookup, prev_frame_idx)
                    cam_cur = get_camera(camera_lookup, fi)

                    warp_depth = make_warp_candidate_depth_y(
                        prev_recon_y=prev_recon_y,
                        cam_prev=cam_prev,
                        cam_cur=cam_cur,
                        w=args.width,
                        h=args.height,
                ...
                print(
                    ...
                    f"warp={sm['warp_candidate_available']} | "
                    f"MAE={sm['mae']:.4f} | "
                    f"PSNR={sm['psnr']:.3f} | "
