...
def rodrigues_torch(rvec: torch.Tensor) -> torch.Tensor:
    ...
    theta2_safe = torch.clamp(theta2, min=1e-12)
    theta = torch.sqrt(theta2_safe)
    small = theta2 < 1e-8
    A_large = torch.sin(theta) / theta
    B_large = (1.0 - torch.cos(theta)) / theta2_safe
...
def so3_log_torch(R: torch.Tensor, eps: float = 1e-7) -> torch.Tensor:
    """Batched SO(3) logarithm. R: [...,3,3], returns [...,3].

    Uses atan2(norm(skew), trace) instead of acos(trace) to avoid NaN gradients
    near identity rotations during smoothness/predictor-loss optimization.
    """
    vee = torch.stack([
        R[..., 2, 1] - R[..., 1, 2],
        R[..., 0, 2] - R[..., 2, 0],
    ...
    denom = torch.clamp(2.0 * sin_theta, min=eps)
    scale_general = theta / denom
    # For small rotations, log(R) ~= 0.5 * vee(R - R^T).
    scale_small = 0.5 + (theta * theta) / 12.0
    scale = torch.where(sin_theta < 1e-4, scale_small, scale_general)
    return scale[..., None] * vee


def camera_center_from_R_t_torch(R: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
    """For W2C X_cam=R X_world+t, camera center C=-R^T t."""
    return -(R.transpose(-1, -2) @ t[..., None]).squeeze(-1)


def t_from_R_C_torch(R: torch.Tensor, C: torch.Tensor) -> torch.Tensor:
    """For W2C X_cam=R(X_world-C), t=-R C."""
    return -(R @ C[..., None]).squeeze(-1)


def trajectory_smoothness_loss(
    R_all: torch.Tensor,
    C_all: torch.Tensor,
    center_acc_weight: float,
    center_vel_weight: float,
    rot_acc_weight: float,
    rot_vel_weight: float,
) -> torch.Tensor:
    """Smooth camera center acceleration and SO(3) angular velocity."""
    loss = torch.zeros((), dtype=R_all.dtype, device=R_all.device)
    n = R_all.shape[0]
    if n >= 3 and center_acc_weight > 0:
        C_acc = C_all[2:] - 2.0 * C_all[1:-1] + C_all[:-2]
        loss = loss + float(center_acc_weight) * torch.mean(C_acc * C_acc)
    if n >= 2 and center_vel_weight > 0:
        C_vel = C_all[1:] - C_all[:-1]
        loss = loss + float(center_vel_weight) * torch.mean(C_vel * C_vel)
    if n >= 2:
        # Increment from frame i-1 orientation to frame i orientation.
        R_inc = R_all[1:] @ R_all[:-1].transpose(-1, -2)
        omega = so3_log_torch(R_inc)
        if rot_vel_weight > 0:
            loss = loss + float(rot_vel_weight) * torch.mean(omega * omega)
        if n >= 3 and rot_acc_weight > 0:
...
class OptimConfig:
    ...
    stage4_lr_c: float
    stage4_lr_nn: float
    stage4_pair_weight: float
    stage4_center_acc: float
    stage4_center_vel: float
    stage4_rot_acc: float
    ...
    spline_ctrl_count: int
    spline_pair_weight_stage1: float
    spline_pair_weight_stage2: float
    spline_pair_weight_stage3: float
    spline_pred_weight_stage1: float
    spline_pred_weight_stage2: float
    spline_pred_weight_stage3: float
    spline_ctrl_prior_stage2: float
    spline_ctrl_prior_stage3: float
    spline_ctrl_smooth_r: float
    spline_ctrl_smooth_c: float
    spline_grad_clip: float
    nn_grid_cell: int
    nn_hidden_ch: int
    nn_latent_ch: int
    nn_layers: int
...
def optimize_stage1_rt(
    ...
    for it in range(1, cfg.stage1_iters + 1):
        ...
        loss.backward()
        if cfg.spline_grad_clip > 0:
            torch.nn.utils.clip_grad_norm_([p for group in opt.param_groups for p in group["params"] if p.grad is not None], float(cfg.spline_grad_clip))
        opt.step()
...
def optimize_stage4_trajectory_smooth(
    pair_cache: list[PairCacheTorch],
    r_stage3_np: np.ndarray,
    t_stage3_np: np.ndarray,
    net: DepthCorrectionNet,
    latent_base: torch.Tensor,
    g_base_np: np.ndarray,
    ...
    device: torch.device,
    dtype: torch.dtype,
) -> tuple[np.ndarray, np.ndarray, torch.Tensor, np.ndarray, list[dict[str, float]]]:
    """
    Stage 4: optimize camera-center trajectory smoothness while preserving projection.

    Variables:
      - r_i: absolute W2C orientation in SO(3) rvec form
      - C_i: camera center, where t_i = -R_i C_i
      - optionally latent grid only; NN weights are frozen
    ...
    with torch.no_grad():
        R_base = rodrigues_torch(r_base)
        C_base = camera_center_from_R_t_torch(R_base, t_base)

    r0_fixed = r_base[0].detach()
    C0_fixed = C_base[0].detach()
    r_free = torch.nn.Parameter(r_base[1:].clone())
    C_free = torch.nn.Parameter(C_base[1:].clone())

    # Freeze NN weights. Optionally fine-tune only the per-frame latent field.
    for p in net.parameters():
        p.requires_grad_(False)
    if cfg.stage4_lr_nn > 0:
        latent = torch.nn.Parameter(latent_base.clone().detach())
        opt_params: list[dict[str, Any]] = [
            {"params": [r_free], "lr": cfg.stage4_lr_r},
            {"params": [C_free], "lr": cfg.stage4_lr_c},
            {"params": [latent], "lr": cfg.stage4_lr_nn},
        ]
    else:
        latent = latent_base.clone().detach()
        opt_params = [
            {"params": [r_free], "lr": cfg.stage4_lr_r},
            {"params": [C_free], "lr": cfg.stage4_lr_c},
        ]
    ...
    hist: list[dict[str, float]] = []
    for it in range(1, cfg.stage4_iters + 1):
        opt.zero_grad(set_to_none=True)
        r_all = torch.cat([r0_fixed[None, :], r_free], dim=0)
        C_all = torch.cat([C0_fixed[None, :], C_free], dim=0)
        R_all = rodrigues_torch(r_all)
        t_all = t_from_R_C_torch(R_all, C_all)
        ...
        loss_smooth = trajectory_smoothness_loss(
            R_all,
            C_all,
            center_acc_weight=cfg.stage4_center_acc,
            center_vel_weight=cfg.stage4_center_vel,
        ...
        if cfg.stage4_pose_prior > 0:
            loss_reg = loss_reg + float(cfg.stage4_pose_prior) * (
                torch.mean((r_all - r_base) ** 2) + torch.mean((C_all - C_base) ** 2)
            )
        if cfg.stage4_lr_nn > 0:
            loss_reg = loss_reg + depth_nn_regularization(g_maps, wx, wy, cfg.depth_l2, cfg.depth_spatial_smooth, cfg.depth_temporal_smooth, cfg.depth_edge_alpha)
        ...
        if it == 1 or it == cfg.stage4_iters or (cfg.print_every > 0 and it % cfg.print_every == 0):
            rec = {
                "iter": it,
                "loss": float(loss.detach().cpu()),
                "pair_loss": float(loss_pair.detach().cpu()),
    ...
    with torch.no_grad():
        ...
        if cfg.stage4_lr_nn > 0:
            ...
            g_maps = g_base
        g_np = g_maps.detach().cpu().numpy().astype(np.float32)
    return r_all.detach().cpu().numpy(), t_all.detach().cpu().numpy(), latent.detach().clone(), g_np, hist



# ============================================================
# Low-DOF spline trajectory model
# ============================================================

def camera_centers_from_r_t_np(r_np: np.ndarray, t_np: np.ndarray) -> np.ndarray:
    R = np.stack([R_from_rvec_np(r_np[i]) for i in range(len(r_np))], axis=0)
    C = np.empty_like(t_np, dtype=np.float64)
    for i in range(len(r_np)):
        C[i] = -(R[i].T @ t_np[i])
    return C.astype(np.float64)


def init_controls_from_sequence_np(seq: np.ndarray, ctrl_count: int) -> np.ndarray:
    """Sample a smooth low-DOF control sequence from a per-frame sequence."""
    seq = np.asarray(seq, dtype=np.float64)
    n, d = seq.shape
    m = max(2, min(int(ctrl_count), n))
    src_x = np.arange(n, dtype=np.float64)
    ctrl_x = np.linspace(0.0, float(n - 1), m, dtype=np.float64)
    out = np.empty((m, d), dtype=np.float64)
    for k in range(d):
        out[:, k] = np.interp(ctrl_x, src_x, seq[:, k])
    return out


def interpolate_controls_torch(ctrl: torch.Tensor, n: int) -> torch.Tensor:
    """Linear differentiable interpolation. ctrl [M,D] -> seq [N,D]."""
    if ctrl.ndim != 2:
        raise ValueError(f"ctrl must be [M,D], got {tuple(ctrl.shape)}")
    if ctrl.shape[0] == int(n):
        return ctrl
    x = ctrl.transpose(0, 1).unsqueeze(0)  # [1,D,M]
    y = F.interpolate(x, size=int(n), mode="linear", align_corners=True)
    return y.squeeze(0).transpose(0, 1).contiguous()


def build_spline_pose_torch(r_ctrl: torch.Tensor, C_ctrl: torch.Tensor, n: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    r_all = interpolate_controls_torch(r_ctrl, n)
    C_all = interpolate_controls_torch(C_ctrl, n)
    R_all = rodrigues_torch(r_all)
    t_all = t_from_R_C_torch(R_all, C_all)
    return r_all, C_all, R_all, t_all


def control_second_diff_loss(ctrl: torch.Tensor, weight: float) -> torch.Tensor:
    if float(weight) <= 0 or ctrl.shape[0] < 3:
        return torch.zeros((), dtype=ctrl.dtype, device=ctrl.device)
    dd = ctrl[2:] - 2.0 * ctrl[1:-1] + ctrl[:-2]
    return float(weight) * torch.mean(dd * dd)


def make_spline_control_tensors(
    r_init_np: np.ndarray,
    t_init_np: np.ndarray,
    ctrl_count: int,
    device: torch.device,
    ...
    C_init_np = camera_centers_from_r_t_np(r_init_np, t_init_np)
    r_ctrl_np = init_controls_from_sequence_np(r_init_np, ctrl_count)
    C_ctrl_np = init_controls_from_sequence_np(C_init_np, ctrl_count)
    return (
        torch.tensor(r_ctrl_np, device=device, dtype=dtype),
        torch.tensor(C_ctrl_np, device=device, dtype=dtype),
    )


def optimize_spline_stage(
    stage_name: str,
    pair_cache: list[PairCacheTorch],
    r_ctrl_init: torch.Tensor,
    C_ctrl_init: torch.Tensor,
    K_fixed: np.ndarray,
    cfg: OptimConfig,
    width: int,
    height: int,
    ...
    pair_weight: float,
    pred_weight: float,
    ctrl_prior_weight: float,
    traj_center_acc: float,
    traj_rot_acc: float,
    net: DepthCorrectionNet | None = None,
    latent_init: torch.Tensor | None = None,
    features: torch.Tensor | None = None,
    wx: torch.Tensor | None = None,
    wy: torch.Tensor | None = None,
    lr_nn: float = 0.0,
    g_base_np: np.ndarray | None = None,
    depth_prior: float = 0.0,
) -> tuple[np.ndarray, np.ndarray, torch.Tensor, torch.Tensor, DepthCorrectionNet | None, torch.Tensor | None, np.ndarray | None, list[dict[str, float]]]:
    """
    Optimize a low-DOF camera trajectory, represented by control points of
    rvec_abs and camera center C_abs. The output still expands to per-frame
    rvec_abs/tvec_abs for downstream compatibility.
    """
    log(f"{stage_name}: low-DOF spline R/C trajectory optimization")
    n = pair_cache[0].target_xy.new_zeros(()).new_tensor(0).shape[0] if False else None
    # Infer frame count from the max pair index, robust to arbitrary pair list ordering.
    frame_count = 1 + max(max(c.spec.target, c.spec.ref) for c in pair_cache)
    K_fixed_t = torch.tensor(K_fixed, device=device, dtype=dtype)

    r_ctrl_base = r_ctrl_init.detach().clone()
    C_ctrl_base = C_ctrl_init.detach().clone()

    # Fix the first control point to preserve RAP anchor gauge. Optimize only remaining controls.
    r0_ctrl = r_ctrl_base[0].detach()
    C0_ctrl = C_ctrl_base[0].detach()
    r_free = torch.nn.Parameter(r_ctrl_base[1:].clone())
    C_free = torch.nn.Parameter(C_ctrl_base[1:].clone())

    params: list[dict[str, Any]] = [
        {"params": [r_free], "lr": float(lr_r)},
        {"params": [C_free], "lr": float(lr_C)},
    ]

    use_nn = net is not None and features is not None and latent_init is not None
    if use_nn:
        latent = torch.nn.Parameter(latent_init.clone().detach())
        if lr_nn > 0:
            for p in net.parameters():
                p.requires_grad_(True)
            params.append({"params": list(net.parameters()) + [latent], "lr": float(lr_nn)})
        else:
            for p in net.parameters():
                p.requires_grad_(False)
            params.append({"params": [latent], "lr": 0.0})
    else:
        latent = None

    g_base = torch.tensor(g_base_np, device=device, dtype=dtype) if g_base_np is not None else None
    opt = torch.optim.Adam(params)
    hist: list[dict[str, float]] = []

    for it in range(1, int(iters) + 1):
        opt.zero_grad(set_to_none=True)
        r_ctrl = torch.cat([r0_ctrl[None, :], r_free], dim=0)
        C_ctrl = torch.cat([C0_ctrl[None, :], C_free], dim=0)
        r_all, C_all, R_all, t_all = build_spline_pose_torch(r_ctrl, C_ctrl, frame_count)

        if use_nn:
            g_maps = net(features, latent, cfg.depth_max_log_scale)  # type: ignore[arg-type]
        else:
            g_maps = None
        ...
        loss_pred = torch.zeros_like(loss_pair)
        pred_stats = {"pred_loss_r": 0.0, "pred_loss_t": 0.0, "mean_abs_q_r": 0.0, "mean_abs_q_t": 0.0}
        if pred_weight > 0:
            loss_pred, pred_stats = actual_predictor_residual_surrogate_loss(
                R_all,
                t_all,
        ...
        loss_traj = trajectory_smoothness_loss(
            R_all,
            C_all,
            center_acc_weight=traj_center_acc,
            center_vel_weight=0.0,
            rot_acc_weight=traj_rot_acc,
            rot_vel_weight=0.0,
        )
        loss_ctrl = control_second_diff_loss(r_ctrl, cfg.spline_ctrl_smooth_r) + control_second_diff_loss(C_ctrl, cfg.spline_ctrl_smooth_c)
        loss_prior = torch.zeros_like(loss_pair)
        if ctrl_prior_weight > 0:
            loss_prior = float(ctrl_prior_weight) * (
                torch.mean((r_ctrl - r_ctrl_base) ** 2) + torch.mean((C_ctrl - C_ctrl_base) ** 2)
            )

        loss_depth = torch.zeros_like(loss_pair)
        if use_nn and g_maps is not None and wx is not None and wy is not None:
            loss_depth = loss_depth + depth_nn_regularization(g_maps, wx, wy, cfg.depth_l2, cfg.depth_spatial_smooth, cfg.depth_temporal_smooth, cfg.depth_edge_alpha)
            if depth_prior > 0 and g_base is not None:
                loss_depth = loss_depth + float(depth_prior) * torch.mean((g_maps - g_base) ** 2)

        loss = float(pair_weight) * loss_pair + float(pred_weight) * loss_pred + loss_traj + loss_ctrl + loss_prior + loss_depth
        loss.backward()
        if cfg.spline_grad_clip > 0:
            torch.nn.utils.clip_grad_norm_([p for group in opt.param_groups for p in group["params"] if p.grad is not None], float(cfg.spline_grad_clip))
        opt.step()

        if it == 1 or it == int(iters) or (cfg.print_every > 0 and it % cfg.print_every == 0):
            rec = {
                "iter": it,
                "loss": float(loss.detach().cpu()),
                "pair_loss": float(loss_pair.detach().cpu()),
                "pred_loss": float(loss_pred.detach().cpu()),
                "pred_loss_r": float(pred_stats.get("pred_loss_r", 0.0)),
                "pred_loss_t": float(pred_stats.get("pred_loss_t", 0.0)),
                "mean_abs_q_r": float(pred_stats.get("mean_abs_q_r", 0.0)),
                "mean_abs_q_t": float(pred_stats.get("mean_abs_q_t", 0.0)),
                "traj_loss": float(loss_traj.detach().cpu()),
                "ctrl_loss": float(loss_ctrl.detach().cpu()),
                "prior_loss": float(loss_prior.detach().cpu()),
                "depth_loss": float(loss_depth.detach().cpu()),
            }
            hist.append(rec)
            log(
                f"{stage_name} iter {it:04d}/{iters}: "
                f"loss={rec['loss']:.6f}, pair={rec['pair_loss']:.6f}, "
                f"pred={rec['pred_loss']:.6f}, qR={rec['mean_abs_q_r']:.3f}, qT={rec['mean_abs_q_t']:.3f}, "
                f"traj={rec['traj_loss']:.6f}, depth={rec['depth_loss']:.6f}"
            )
    ...
    with torch.no_grad():
        ...
        if use_nn:
            ...
            g_np = None
    return (
        r_all.detach().cpu().numpy(),
        t_all.detach().cpu().numpy(),
        r_ctrl.detach(),
        C_ctrl.detach(),
        net,
        latent.detach().clone() if latent is not None else None,
        g_np,
        hist,
    )
...
def run(args: argparse.Namespace) -> None:
    ...
    # Keep suffixes compatible with convert_vggt_or_canonical_to_camparam.py.
    ...
    cfg = OptimConfig(
        ...
        stage4_lr_r=args.stage4_lr_r,
        stage4_lr_c=args.stage4_lr_c,
        stage4_lr_nn=args.stage4_lr_nn,
        stage4_pair_weight=args.stage4_pair_weight,
        stage4_center_acc=args.stage4_center_acc,
        stage4_center_vel=args.stage4_center_vel,
        stage4_rot_acc=args.stage4_rot_acc,
        ...
        spline_ctrl_count=args.spline_ctrl_count,
        spline_pair_weight_stage1=args.spline_pair_weight_stage1,
        spline_pair_weight_stage2=args.spline_pair_weight_stage2,
        spline_pair_weight_stage3=args.spline_pair_weight_stage3,
        spline_pred_weight_stage1=args.spline_pred_weight_stage1,
        spline_pred_weight_stage2=args.spline_pred_weight_stage2,
        spline_pred_weight_stage3=args.spline_pred_weight_stage3,
        spline_ctrl_prior_stage2=args.spline_ctrl_prior_stage2,
        spline_ctrl_prior_stage3=args.spline_ctrl_prior_stage3,
        spline_ctrl_smooth_r=args.spline_ctrl_smooth_r,
        spline_ctrl_smooth_c=args.spline_ctrl_smooth_c,
        spline_grad_clip=args.spline_grad_clip,
    ...
    # Use the same depth-scale normalization as the downstream coding script for predictor loss.
    if cfg.stage4_pred_depth_scale_real > 0:
        pred_depth_scale_real = float(cfg.stage4_pred_depth_scale_real)
    else:
        pred_depth_scale_real = float(choose_depth_scale_fixed_point(depth, cfg.depth_scale_percentile, cfg.depth_scale_precision, 10)["depth_scale_real"])
    log(f"Predictor depth_scale_real for t normalization: {pred_depth_scale_real:.9g}")
    log(f"Predictor fine steps: r_step={cfg.stage4_pred_r_step:.9g}, t_step_norm={cfg.stage4_pred_t_step_norm:.9g}")

    log("Initializing low-DOF spline controls from initial pose sequence")
    r_ctrl0, C_ctrl0 = make_spline_control_tensors(r_init, t_init, cfg.spline_ctrl_count, device, dtype)
    log(f"Spline control count: {r_ctrl0.shape[0]} for {n} frames")

    # ------------------------------------------------------------------
    # Stage 1: smooth low-DOF R/C only, original depth.
    # ------------------------------------------------------------------
    if cfg.stage1_iters > 0:
        r_stage1, t_stage1, r_ctrl1, C_ctrl1, _, _, _, stage1_hist = optimize_spline_stage(
            "Stage 1", pair_cache_t, r_ctrl0, C_ctrl0, K_fixed, cfg, w, h, device, dtype,
            pred_depth_scale_real=pred_depth_scale_real,
            iters=cfg.stage1_iters,
            lr_r=cfg.stage1_lr_r,
            lr_C=cfg.stage1_lr_t,
            pair_weight=cfg.spline_pair_weight_stage1,
            pred_weight=cfg.spline_pred_weight_stage1,
            ctrl_prior_weight=cfg.stage1_rot_prior,
            traj_center_acc=cfg.stage4_center_acc,
            traj_rot_acc=cfg.stage4_rot_acc,
            net=None,
        )
    else:
        log("Stage 1 skipped because --stage1-iters 0")
        with torch.no_grad():
            r_all_t, C_all_t, R_all_t, t_all_t = build_spline_pose_torch(r_ctrl0, C_ctrl0, n)
        r_stage1 = r_all_t.detach().cpu().numpy()
        t_stage1 = t_all_t.detach().cpu().numpy()
        r_ctrl1, C_ctrl1 = r_ctrl0.detach(), C_ctrl0.detach()
        stage1_hist = []
    stage1_eval = evaluate_on_cache("stage1_spline_pose", pair_cache_np, r_stage1, t_stage1, K_fixed, w, h, cfg.z_sign, None)
    log(f"Stage1 spline EPE: mean={stage1_eval['mean_of_mean_epe']:.6f}, p95={stage1_eval['mean_of_p95_epe']:.6f}")
    stage1_pred_bits = estimate_actual_history_pred_bits_np(
        r_stage1, t_stage1, pred_depth_scale_real,
    ...
    log(f"Stage1 actual-predictor bits: avg={stage1_pred_bits['avg_bits_per_frame']:.3f}, "
        f"r={stage1_pred_bits['avg_r_bits_per_frame']:.3f}, t={stage1_pred_bits['avg_t_bits_per_frame']:.3f}")

    # ------------------------------------------------------------------
    # Stage 2: same smooth spline trajectory, add depth NN to absorb projection mismatch.
    # ------------------------------------------------------------------
    ...
    latent0 = torch.zeros((n, cfg.nn_latent_ch, features.shape[2], features.shape[3]), device=device, dtype=dtype)
    if cfg.stage2_iters > 0:
        r_stage2, t_stage2, r_ctrl2, C_ctrl2, net, latent_stage2, g_stage2, stage2_hist = optimize_spline_stage(
            "Stage 2", pair_cache_t, r_ctrl1, C_ctrl1, K_fixed, cfg, w, h, device, dtype,
            pred_depth_scale_real=pred_depth_scale_real,
            iters=cfg.stage2_iters,
            lr_r=cfg.stage1_lr_r * 0.5,
            lr_C=cfg.stage2_lr_t,
            pair_weight=cfg.spline_pair_weight_stage2,
            pred_weight=cfg.spline_pred_weight_stage2,
            ctrl_prior_weight=cfg.spline_ctrl_prior_stage2,
            traj_center_acc=cfg.stage4_center_acc,
            traj_rot_acc=cfg.stage4_rot_acc,
            net=net,
            latent_init=latent0,
            ...
            wx=wx_t,
            wy=wy_t,
            lr_nn=cfg.stage2_lr_nn,
            g_base_np=None,
            depth_prior=0.0,
        )
    else:
        log("Stage 2 skipped because --stage2-iters 0")
        r_stage2, t_stage2 = r_stage1.copy(), t_stage1.copy()
        r_ctrl2, C_ctrl2 = r_ctrl1, C_ctrl1
        latent_stage2 = latent0
        g_stage2 = np.zeros((n, 1, features.shape[2], features.shape[3]), dtype=np.float32)
        stage2_hist = []
    stage2_eval = evaluate_on_cache("stage2_spline_pose_nn_depth", pair_cache_np, r_stage2, t_stage2, K_fixed, w, h, cfg.z_sign, g_stage2)
    log(f"Stage2 spline+NN EPE: mean={stage2_eval['mean_of_mean_epe']:.6f}, p95={stage2_eval['mean_of_p95_epe']:.6f}")
    stage2_pred_bits = estimate_actual_history_pred_bits_np(
        r_stage2, t_stage2, pred_depth_scale_real,
        pred_n=cfg.stage4_pred_n, pred_degree=cfg.stage4_pred_degree,
        r_step=cfg.stage4_pred_r_step, t_step_norm=cfg.stage4_pred_t_step_norm,
        ext_bits=cfg.stage4_pred_ext_bits,
    )
    log(f"Stage2 actual-predictor bits: avg={stage2_pred_bits['avg_bits_per_frame']:.3f}, "
        f"r={stage2_pred_bits['avg_r_bits_per_frame']:.3f}, t={stage2_pred_bits['avg_t_bits_per_frame']:.3f}")

    # ------------------------------------------------------------------
    # Stage 3: final smooth spline + NN joint fine-tune. No free per-frame pose variables.
    # ------------------------------------------------------------------
    if cfg.stage3_iters > 0:
        r_stage3, t_stage3, r_ctrl3, C_ctrl3, net, latent_stage3, g_stage3, stage3_hist = optimize_spline_stage(
            "Stage 3", pair_cache_t, r_ctrl2, C_ctrl2, K_fixed, cfg, w, h, device, dtype,
            pred_depth_scale_real=pred_depth_scale_real,
            iters=cfg.stage3_iters,
            lr_r=cfg.stage3_lr_r,
            lr_C=cfg.stage3_lr_t,
            pair_weight=cfg.spline_pair_weight_stage3,
            pred_weight=cfg.spline_pred_weight_stage3,
            ctrl_prior_weight=cfg.spline_ctrl_prior_stage3,
            traj_center_acc=cfg.stage4_center_acc,
            traj_rot_acc=cfg.stage4_rot_acc,
            net=net,
            latent_init=latent_stage2,
            features=features,
            wx=wx_t,
            wy=wy_t,
            lr_nn=cfg.stage3_lr_nn,
            g_base_np=g_stage2,
            depth_prior=cfg.stage3_depth_prior,
        )
    else:
        log("Stage 3 skipped because --stage3-iters 0")
        r_stage3, t_stage3 = r_stage2.copy(), t_stage2.copy()
        r_ctrl3, C_ctrl3 = r_ctrl2, C_ctrl2
        latent_stage3 = latent_stage2
        g_stage3 = g_stage2
        stage3_hist = []

    stage3_eval = evaluate_on_cache("stage3_spline_final", pair_cache_np, r_stage3, t_stage3, K_fixed, w, h, cfg.z_sign, g_stage3)
    log(f"Stage3 final spline EPE: mean={stage3_eval['mean_of_mean_epe']:.6f}, p95={stage3_eval['mean_of_p95_epe']:.6f}")
    ...
    # Final outputs keep the same keys expected by the converter.
    r_final = r_stage3
    t_final = t_stage3
    g_final = g_stage3
    final_eval = stage3_eval
    final_pred_bits = stage3_pred_bits

    log("Applying final NN depth correction and writing depth YUV")
    depth_canonical = apply_depth_correction_full(depth, g_final, h, w)
    ...
    write_camera_jsonl(out_jsonl, npz_path, camera_jsonl_path, frame_indices, K_fixed, r_final, t_final, depth_yuv_meta, cfg, pair_list)
    ...
    payload: dict[str, Any] = {
        ...
        "rvec_abs_stage2_t_nn": r_stage2.astype(np.float32),
        "tvec_abs_stage2_t_nn": t_stage2.astype(np.float32),
        "rvec_abs_stage3_joint": r_stage3.astype(np.float32),
        "tvec_abs_stage3_joint": t_stage3.astype(np.float32),
        "rvec_abs_stage4_smooth": r_final.astype(np.float32),
        "tvec_abs_stage4_smooth": t_final.astype(np.float32),
        "rvec_abs_final": r_final.astype(np.float32),
        "tvec_abs_final": t_final.astype(np.float32),
        "rvec_spline_ctrl_stage1": r_ctrl1.detach().cpu().numpy().astype(np.float32),
        "C_spline_ctrl_stage1": C_ctrl1.detach().cpu().numpy().astype(np.float32),
        "rvec_spline_ctrl_stage2": r_ctrl2.detach().cpu().numpy().astype(np.float32),
        "C_spline_ctrl_stage2": C_ctrl2.detach().cpu().numpy().astype(np.float32),
        "rvec_spline_ctrl_final": r_ctrl3.detach().cpu().numpy().astype(np.float32),
        "C_spline_ctrl_final": C_ctrl3.detach().cpu().numpy().astype(np.float32),
        "depth_canonical": depth_canonical.astype(np.float32),
        "depth_log_scale_grid_stage2": g_stage2.astype(np.float32),
        "depth_log_scale_grid_stage3": g_stage3.astype(np.float32),
        "depth_log_scale_grid_stage4": g_final.astype(np.float32),
        "depth_log_scale_grid_final": g_final.astype(np.float32),
        "depth_log_scale_grid_shape": np.asarray(g_final.shape, dtype=np.int32),
        ...
        "stage4_eval_json": np.asarray(json.dumps(final_eval, ensure_ascii=False), dtype=object),
        "stage1_actual_pred_bits_json": np.asarray(json.dumps(stage1_pred_bits, ensure_ascii=False), dtype=object),
        "stage2_actual_pred_bits_json": np.asarray(json.dumps(stage2_pred_bits, ensure_ascii=False), dtype=object),
        "stage3_actual_pred_bits_json": np.asarray(json.dumps(stage3_pred_bits, ensure_ascii=False), dtype=object),
        "stage4_actual_pred_bits_json": np.asarray(json.dumps(final_pred_bits, ensure_ascii=False), dtype=object),
        ...
        "stage4_history_json": np.asarray(json.dumps([], ensure_ascii=False), dtype=object),
    ...
    if args.save_nn_state:
        ...
        torch.save({
            ...
            "latent": latent_stage3.detach().cpu() if latent_stage3 is not None else None,
    ...
    manifest = {
        ...
        "eval": {
            "init": init_eval,
            "stage1_spline_pose": stage1_eval,
            "stage2_spline_pose_nn_depth": stage2_eval,
            "stage3_spline_final": stage3_eval,
        },
        "actual_predictor_bit_estimate": {
            "stage1": stage1_pred_bits,
            "stage2": stage2_pred_bits,
            "stage3_final": final_pred_bits,
        },
        "depth_yuv": depth_yuv_meta,
        "notes": [
            "Pair direction is target->reference; backward projection uses target-frame depth.",
            "This version does not optimize free per-frame R/t. It parameterizes absolute R and camera center C by low-DOF linear spline control points from the start.",
            ...
            "Output NPZ keys K_fixed/depth_canonical/rvec_abs_final/tvec_abs_final are kept compatible with convert_vggt_or_canonical_to_camparam.py.",
            "Predictor residual loss and bit estimates use current_to_previous param6=[rvec,tvec/depth_scale_real] with fine defaults r_step=1/512 and t_step_norm=1/128.",
        ],
    }
    ...
    print("============================================================")
    print("Fixed-K spline GOP NN-depth optimization done")
    print("============================================================")
    ...
    print(f"spline ctrl     : {cfg.spline_ctrl_count}")
    print(f"NN grid         : {g_final.shape[2]}x{g_final.shape[3]}  cell~{cfg.nn_grid_cell}")
    print("------------------------------------------------------------")
    ...
    print("------------------------------------------------------------")
    print(f"stage1 pred bits/frame: {stage1_pred_bits['avg_bits_per_frame']:.3f} "
          f"(R={stage1_pred_bits['avg_r_bits_per_frame']:.3f}, T={stage1_pred_bits['avg_t_bits_per_frame']:.3f})")
    print(f"stage2 pred bits/frame: {stage2_pred_bits['avg_bits_per_frame']:.3f} "
          f"(R={stage2_pred_bits['avg_r_bits_per_frame']:.3f}, T={stage2_pred_bits['avg_t_bits_per_frame']:.3f})")
    print(f"stage3 pred bits/frame: {final_pred_bits['avg_bits_per_frame']:.3f} "
          f"(R={final_pred_bits['avg_r_bits_per_frame']:.3f}, T={final_pred_bits['avg_t_bits_per_frame']:.3f})")
    print("------------------------------------------------------------")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Fixed-K low-DOF spline R/C trajectory + constrained tiny-NN depth correction for codec input")
    ...
    # Spline trajectory model. The final per-frame R/t is generated from these low-DOF controls.
    p.add_argument("--spline-ctrl-count", type=int, default=6, help="Number of low-DOF control points for absolute rvec and camera center C")
    p.add_argument("--spline-pair-weight-stage1", type=float, default=1.0)
    p.add_argument("--spline-pair-weight-stage2", type=float, default=8.0)
    p.add_argument("--spline-pair-weight-stage3", type=float, default=15.0)
    p.add_argument("--spline-pred-weight-stage1", type=float, default=0.001, help="Predictor residual loss weight for spline pose-only stage")
    p.add_argument("--spline-pred-weight-stage2", type=float, default=0.003, help="Predictor residual loss weight for spline+NN stage")
    p.add_argument("--spline-pred-weight-stage3", type=float, default=0.005, help="Predictor residual loss weight for final spline+NN stage")
    p.add_argument("--spline-ctrl-prior-stage2", type=float, default=1e-4)
    p.add_argument("--spline-ctrl-prior-stage3", type=float, default=5e-4)
    p.add_argument("--spline-ctrl-smooth-r", type=float, default=0.0, help="Second-diff penalty on rotation control points")
    p.add_argument("--spline-ctrl-smooth-c", type=float, default=0.0, help="Second-diff penalty on center control points")
    p.add_argument("--spline-grad-clip", type=float, default=1.0, help="Gradient norm clip for spline/control optimization; helps with fine quant-step pred loss")

    # Stage 1.
    p.add_argument("--stage1-iters", type=int, default=400)
    p.add_argument("--stage1-lr-r", type=float, default=5e-4)
    p.add_argument("--stage1-lr-t", type=float, default=5e-4)
    p.add_argument("--stage1-rot-prior", type=float, default=1e-6)
    p.add_argument("--stage1-t-prior", type=float, default=1e-6)

    # Stage 2.
    p.add_argument("--stage2-iters", type=int, default=500)
    ...
    p.add_argument("--stage3-iters", type=int, default=300)
    p.add_argument("--stage3-lr-r", type=float, default=5e-5)
    p.add_argument("--stage3-lr-t", type=float, default=5e-5)
    ...
    # Stage 4: constrained trajectory smoothing from Stage 3.
    p.add_argument("--stage4-iters", type=int, default=200)
    p.add_argument("--stage4-lr-r", type=float, default=2e-5)
    p.add_argument("--stage4-lr-c", type=float, default=2e-5)
    p.add_argument("--stage4-lr-nn", type=float, default=0.0, help="If >0, fine-tune only NN latent grid; NN weights stay frozen")
    p.add_argument("--stage4-pair-weight", type=float, default=10.0, help="Projection-preservation weight. Increase if EPE drifts.")
    p.add_argument("--stage4-center-acc", type=float, default=1.0, help="Camera-center second-difference smoothness")
    p.add_argument("--stage4-center-vel", type=float, default=0.0, help="Optional camera-center velocity damping")
    p.add_argument("--stage4-rot-acc", type=float, default=1.0, help="SO(3) angular-velocity second-difference smoothness")
    ...
    p.add_argument("--stage4-pred-r-step", type=float, default=1.0/512.0, help="Match downstream coding script --r-step. Default is 1/512, not 1/16.")
    p.add_argument("--stage4-pred-t-step-norm", type=float, default=1.0/128.0, help="Match downstream coding script --t-step-norm. Default is 1/128, not 1/4.")
...
def main() -> None:
    ...
    if args.spline_ctrl_count < 2:
        raise ValueError("--spline-ctrl-count must be >= 2")
