...
Example:
  python run_vggt_omega_yuv.py \
    ...
    --pix-fmt yuv420p10le \
    --start 0 --end 15 \
    --output-prefix out/test_000_015
"""
...
def resize_depth_to_original(
    ...
    return np.pad(crop_depth, ((pad_top, pad_bottom), (pad_left, pad_right)), mode="edge")
...
def quantize_depth_to_y(qsrc_frame: np.ndarray, qmin: float, qmax: float, out_bit_depth: int) -> np.ndarray:
...
def write_depth_yuv420(
    ...
    with open(path, "wb") as f:
        for i in range(n):
            qmin, qmax = compute_quant_range(qsrc, norm_mode, i, fixed_min, fixed_max, clip_percentile)
            ...
            metadata.append(
                {
                    ...
                    "note": (
                        "Y stores depth directly" if mode == "linear" else "Y stores inverse depth = 1/depth"
                    ),
                }
            )
    return metadata
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-depth-pix-fmt", choices=["yuv420p", "yuv420p10le"], default="yuv420p10le")
    ...
    p.add_argument("--depth-min", type=float, default=None, help="q-domain min. For inverse mode this means min(1/depth).")
    p.add_argument("--depth-max", type=float, default=None, help="q-domain max. For inverse mode this means max(1/depth).")
    p.add_argument("--clip-percentile", nargs=2, type=float, default=(0.0, 100.0), metavar=("LO", "HI"))
...
def main() -> None:
    ...
    frame_indices = list(range(args.start, args.end + 1, args.stride))
    src_bit_depth = 8 if pix_fmt == "yuv420p" else 10

    print(f"Reading {len(frame_indices)} frames from {args.yuv}")
    rgb_frames: list[np.ndarray] = []
    ...
    print(f"Model input tensor: {tuple(images.shape)}  (S,C,H,W)")

    if args.save_model_input_rgb:
        rgb_dir = args.output_prefix + "_model_input_rgb"
        os.makedirs(rgb_dir, exist_ok=True)
        for i, t in enumerate(images):
            arr = (t.permute(1, 2, 0).numpy() * 255.0 + 0.5).astype(np.uint8)
            Image.fromarray(arr, mode="RGB").save(os.path.join(rgb_dir, f"{frame_indices[i]:06d}.png"))
    ...
    print(f"Depth model shape: {depth_model.shape}")

    depth_orig = np.stack(
        [resize_depth_to_original(depth_model[i], meta, args.fill_crop) for i in range(len(frame_indices))],
        axis=0,
    ).astype(np.float32)
    conf_orig = np.stack(
        [resize_depth_to_original(conf_model[i], meta, args.fill_crop) for i in range(len(frame_indices))],
        axis=0,
    ).astype(np.float32)

    K_orig = np.stack([intrinsic_to_original(intrinsic_model[i], meta) for i in range(len(frame_indices))], axis=0)

    depth_yuv_path = args.output_prefix + f"_depth_{args.depth_quant_mode}_{out_depth_pix_fmt}.yuv"
    ...
    if args.save_conf_yuv:
        # Confidence is not depth, so always write linear/global normalization.
        conf_yuv_path = args.output_prefix + f"_depth_conf_{out_depth_pix_fmt}.yuv"
        write_depth_yuv420(
            conf_yuv_path,
            conf_orig,
            mode="linear",
    ...
    npz_path = args.output_prefix + "_vggt_omega_outputs.npz"
    np.savez_compressed(
        npz_path,
    ...
    camera_jsonl_path = args.output_prefix + "_camera.jsonl"
    ...
    manifest_path = args.output_prefix + "_manifest.json"
    manifest = {
