...
- RA reference structure is obtained from MV CSV when available; otherwise GOP anchors.
...
- Excluded/unavailable frames are copied from GT; invalid warped pixels and chroma use GT.
"""
...
def fallback_ra_refs(poc: int, start: int, end: int, gop: int) -> Tuple[Optional[int], Optional[int]]:
    g0 = start + ((poc - start) // gop) * gop
    g1 = min(g0 + gop, end - 1)
    if poc == g0:
        prev = g0 - gop
        return (prev if prev >= start else None), None
    if poc == g1:
        return g0, None
    return g0, g1
        ...
        with open(a.video_yuv,"rb") as vf, open(a.input_depth_yuv,"rb") as df, open(temp_path,"wb+") as of:
            for oi, poc in enumerate(order):
                ...
                if poc in excluded:
                    ...
                    fb0, fb1 = fallback_ra_refs(poc, start, end, a.ra_gop_size)
                    r0 = next((r for r in l0s[poc] if start <= r < end and r in cams and r != poc), fb0)
                    r1 = next((r for r in l1s[poc] if start <= r < end and r in cams and r != poc), fb1)
                    if a.coding_order == "sequential":
                        r0 = r0 if r0 is not None else poc - a.default_ref_offset
                        r1 = None
