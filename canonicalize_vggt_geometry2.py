...
def so3_log_torch(R: torch.Tensor, eps: float = 1e-7) -> torch.Tensor:
    ...
    # For very small rotations, log(R) ~= 0.5 * vee(R - R^T).
    small = theta < 1e-4
...
def run(args: argparse.Namespace) -> None:
    ...
    if cfg.stage4_iters > 0:
        ...
        log("Stage 4 skipped because --stage4-iters 0")
        r_stage4 = r_stage3.copy()
        t_stage4 = t_stage3.copy()
        latent_stage4 = latent_stage3
        g_stage4 = g_stage3
        stage4_hist = []
    ...
    write_camera_jsonl(out_jsonl, npz_path, camera_jsonl_path, frame_indices, K_fixed, r_stage4, t_stage4, depth_yuv_meta, cfg, pair_list)
    ...
    payload: dict[str, Any] = {
        ...
        "rvec_abs_stage4_smooth": r_stage4.astype(np.float32),
        "tvec_abs_stage4_smooth": t_stage4.astype(np.float32),
        "rvec_abs_final": r_stage4.astype(np.float32),
        "tvec_abs_final": t_stage4.astype(np.float32),
        ...
        "depth_log_scale_grid_stage4": g_stage4.astype(np.float32),
        "depth_log_scale_grid_final": g_stage4.astype(np.float32),
        "depth_log_scale_grid_shape": np.asarray(g_stage4.shape, dtype=np.int32),
        ...
        "stage4_history_json": np.asarray(json.dumps(stage4_hist, ensure_ascii=False), dtype=object),
    ...
    if args.save_nn_state:
        ...
        torch.save({
            ...
            "latent": latent_stage4.detach().cpu(),
    ...
    print(f"NN grid         : {g_stage4.shape[2]}x{g_stage4.shape[3]}  cell~{cfg.nn_grid_cell}")
    print("------------------------------------------------------------")
